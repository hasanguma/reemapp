...
class NativeSpeechRecognitionShim {
  ...
  async start() {
    if (this._listening) return; // نتجاهل بأمان بدل رمي استثناء، مطابقةً لسلوك المتصفح مع try/catch المحيطة بالنداء
    const plugin = this._getPlugin();
    if (!plugin) {
      console.warn('إضافة التعرف الصوتي الأصلية غير مثبّتة/مُزامَنة (npm install + npx cap sync android).');
    ...
    try {
      if (plugin.checkPermissions && plugin.requestPermissions) {
        ...
        const granted = current && (current.speechRecognition === 'granted' || current.speechRecognition === undefined);
        if (!granted) {
          const requested = await plugin.requestPermissions();
          if (requested && requested.speechRecognition && requested.speechRecognition !== 'granted') {
            this._listening = false;
            if (this.onerror) this.onerror({ error: 'not-allowed' });
            return;
          }
...
function getSpeechErrorMessage(errorCode) {
  switch (errorCode) {
    ...
    case 'service-not-allowed':
      return 'يرجى التأكد من السماح باستخدام الميكروفون من إعدادات المتصفح، ثم حاولي مجدداً 🎙️';
    case 'no-speech':
      return 'لم أسمع أي صوت، حاولي التحدث بوضوح والتأكد من قرب الميكروفون 🎙️';
    case 'audio-capture':
      return 'تعذّر العثور على ميكروفون. يرجى التأكد من توصيله وتفعيله ثم إعادة المحاولة 🎙️';
    case 'network':
      return 'تعذّر الوصول لخدمة التعرف الصوتي بسبب مشكلة في الشبكة 🌐 حاولي مجدداً.';
    default:
      return 'حدث خطأ أثناء التعرف على الصوت. يرجى المحاولة مرة أخرى 🎙️';
  }
}
...
async function handleCallUserSpeech(transcript) {
  ...
  try {
    ...
    console.error('خطأ أثناء المكالمة:', error);
    callConsecutiveErrors += 1;
    if (callConsecutiveErrors >= CALL_MAX_AUTO_RETRIES) {
...
function startCall() {
  if (!SpeechRecognition) {
    appendMessage({ text: 'عذراً، متصفحك لا يدعم خاصية التعرف على الصوت اللازمة للمكالمة الصوتية.', sender: 'bot', isError: true, isMarkdown: false });
    return;
  }
  callActive = true;
  callMicMuted = false;
  ...
  setCallStatus('مرحباً بكِ! تحدثي الآن...', 'idle');
  window.speechSynthesis.cancel();
  restartCallListening();
}
...
function endCall() {
  ...
  window.speechSynthesis.cancel();
  if (callRecognition) {
    try { callRecognition.stop(); } catch (err) { /* تجاهل */ }
  }
